...
def home(request):
    ...
    if user.is_authenticated:
        ...
        profile_data = get_object_or_404(Organizer, idno=request.user.username) if user.is_staff else get_object_or_404(Student, idno=request.user.username)
...
def getSchedule():
    ...
    try:
        schedule[today] = sorted(get_list_or_404(EventSchedule, datePlanned=today), key=lambda attrib: attrib.start_time)
        schedule[tommorrow] = sorted(get_list_or_404(EventSchedule, datePlanned=tommorrow), key=lambda attrib: attrib.start_time)
        schedule[nextday] = sorted(get_list_or_404(EventSchedule, datePlanned=nextday), key=lambda attrib: attrib.start_time)
    except:
        pass
    return schedule